...
if uploaded_key is not None:
    ...
    if sh:
        ...
        # --- CHUẨN HÓA DỮ LIỆU ĐẦU VÀO (QUAN TRỌNG) ---
        
        # 1. Xử lý bảng Chi Phí
        if df_cp.empty:
        ...
        if not df_main.empty:
            ...
            # Ép kiểu số tiền
            cols_to_numeric = [
                "Giá", "Giá HĐ", "SALE THẢO", "SALE NGA", "SALE LINH", 
                "Công ty", "Cá Nhân", "TT cho chủ nhà", "Cọc cho chủ nhà", 
        ...
        with tabs[0]:
            ...
            with st.form("main_form"):
                ...
                with c2_2:
                    ...
                    try: 
                        ngay_het_han_auto = ngay_ky + pd.Timedelta(days=thoi_han*30)
                    except: 
                        ngay_het_han_auto = ngay_ky
                ...
                if st.form_submit_button("💾 LƯU HỢP ĐỒNG MỚI", type="primary"):
                    ...
                    st.session_state['auto'] = {} # Reset dữ liệu
                    time.sleep(1)
                    st.rerun()
        ...
        # ---------------------------------------------------------
        with tabs[1]:
            st.header("📤 Quản lý File Excel (Hợp Đồng)")
            st.subheader("1. Tải file mẫu")
            st.download_button("📥 Tải File Mẫu (.xlsx)", convert_df_to_excel(pd.DataFrame(columns=COLUMNS)), "mau_hop_dong.xlsx")
            
            st.divider()
            
            st.subheader("2. Upload dữ liệu")
        ...
        # ---------------------------------------------------------
        with tabs[2]:
            st.subheader("💸 Quản Lý Chi Phí Nội Bộ (Điện, Nước, Net...)")
            
            # Khu vực máy tính và nhập nhanh
            ...
            if st.button("🧹 Quét & Xóa Dữ Liệu Trùng Lặp", type="secondary"):
                if not df_cp.empty:
                    ...
                    if len(df_clean) < len(df_cp): 
                        ...
                        st.info("👍 Dữ liệu sạch, không có trùng lặp.")
        ...
        with tabs[3]:
            ...
            search_term = st.text_input("🔍 Tìm kiếm nhanh (Tên khách, Phòng...):")
        ...
        # ---------------------------------------------------------
        # TAB 6: QUẢN LÝ CHI PHÍ (DETAIL VIEW)
        # ---------------------------------------------------------
        with tabs[5]:
            ...
            if not df_main.empty:
                ...
                st.dataframe(
                    ...
                    use_container_width=True, 
                    column_config={"Ghi chú": st.column_config.TextColumn(width="medium", help="Thông tin ngày tháng")}
                )
            else: 
        ...
        with tabs[6]:
            ...
            if not df_main.empty:
                ...
                cols_final = [
                    ...
                    "Doanh thu cho thuê", "Tổng Chi Phí Sale", 
                    "Công ty", "Cá Nhân", "Lợi nhuận ròng", "Ghi chú"
                ]
        ...
        with tabs[7]:
            ...
            st.write("Theo dõi dòng tiền thực tế ra/vào túi")
            ...
            if not df_main.empty:
                ...
                # --- BẢNG ---
                st.write("##### 📋 Bảng Chi Tiết Dòng Tiền")
                def explain_cf(row):
                    ...
                    if row['Chi: Vận hành'] > 500000: reasons.append(f"⚠️ Chi vận hành cao ({fmt_vnd(row['Chi: Vận hành'])})")
                    return "; ".join(reasons)
