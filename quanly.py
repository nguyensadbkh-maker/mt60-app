...
def connect_google_sheet(file_obj):
    ...
    try:
        ...
        # Xử lý lỗi xuống dòng trong private_key
...
if uploaded_key is not None:
    ...
    if sh:
        ...
        def load_data(tab_name):
            """Tải dữ liệu từ Tab Google Sheet về"""
            try:
        ...
        def save_data(df, tab_name):
            ...
            try:
                wks = sh.worksheet(tab_name)
                # Chuyển đổi NaN thành chuỗi rỗng để tránh lỗi JSON
                df_save = df.fillna("") 
        ...
        def parse_text_message(text):
            """Phân tích tin nhắn Zalo để lấy dữ liệu"""
            extracted = {}
        ...
        # --- HÀM GỘP DỮ LIỆU THÔNG MINH (LOGIC MỚI) ---
        def gop_du_lieu_phong(df):
            """
            Gộp các dòng có cùng Tòa và Mã căn.
            - Ngày: Lấy Min (Ngày bắt đầu) và Max (Ngày kết thúc).
            - Tiền: Cộng tổng.
            - Giá HĐ/Giá Thuê: Cộng tổng (Xử lý trường hợp dòng này có giá, dòng kia bằng 0).
            """
            if df.empty: return df
            
            # Định nghĩa cách gộp cho từng cột
            agg_rules = {
                'Ngày ký': 'min', 'Ngày hết HĐ': 'max',
                'Ngày in': 'min', 'Ngày out': 'max',
                'Giá HĐ': 'sum', # Cộng giá (vì dòng kia = 0)
                'TT cho chủ nhà': 'sum', 'Cọc cho chủ nhà': 'sum',
                'Tên khách thuê': 'first', # Lấy tên đầu tiên tìm thấy
                'Giá': 'sum', # Giá thuê
            ...
            # Chỉ áp dụng rule cho các cột có trong DataFrame
            final_agg = {k: v for k, v in agg_rules.items() if k in df.columns}
            
            # Nhóm theo Tòa và Mã căn
            cols_group = ['Toà', 'Mã căn']
            # Đảm bảo các cột nhóm tồn tại
        ...
        with st.sidebar:
            ...
            if not df_main.empty:
                ...
                if df_hd.empty and df_kh.empty: 
                    ...
                    if not df_hd.empty:
                        st.error(f"🔴 {len(df_hd)} Hợp đồng cần xử lý gấp")
        ...
        # ==========================================
        # 4. CÁC TAB CHỨC NĂNG (GIAO DIỆN CHÍNH)
        # ==========================================
        tabs = st.tabs([
            "✍️ Nhập Liệu", 
            "📥 Upload Excel", 
        ...
        with tabs[2]:
            ...
            st.divider(); st.subheader("📤 Nhập Chi Phí Bằng Excel")
                ...
                if df_warning_hd.empty: st.success("✅ Không có HĐ sắp hết hạn.")
                    ...
                    for idx, row in df_warning_hd.iterrows():
                        ...
                        with st.expander(f"🔴 {row['Mã căn']} - {row['Tên khách thuê']} ({status})"):
                            ...
                            st.code(f"Chào bạn {row['Tên khách thuê']},\nHợp đồng phòng {row['Mã căn']} sắp hết hạn vào {row['Ngày hết HĐ'].strftime('%d/%m/%Y')}. Vui lòng liên hệ gia hạn.", language=None)
        ...
        with tabs[6]:
            ...
            if not df_main.empty:
                # --- TÍNH TOÁN TRÊN TỪNG DÒNG RỒI MỚI GỘP ---
                df_row_calc = df_main.copy()
                ...
                # Tính các chỉ số cho từng dòng rời rạc trước
                df_row_calc['Tháng HĐ'] = df_row_calc.apply(lambda r: get_months(r['Ngày ký'], r['Ngày hết HĐ']), axis=1)
                df_row_calc['Tổng giá trị HĐ'] = (df_row_calc['Giá HĐ'] * df_row_calc['Tháng HĐ'])
                
                df_row_calc['Tháng Thuê'] = df_row_calc.apply(lambda r: get_months(r['Ngày in'], r['Ngày out']), axis=1)
                df_row_calc['Chi phí vốn (theo khách)'] = (df_row_calc['Giá HĐ'] * df_row_calc['Tháng Thuê'])
                df_row_calc['Doanh thu cho thuê'] = (df_row_calc['Giá'] * df_row_calc['Tháng Thuê'])
                df_row_calc['Tổng Chi Phí Sale'] = df_row_calc['SALE THẢO'] + df_row_calc['SALE NGA'] + df_row_calc['SALE LINH']
                df_row_calc['Lợi nhuận ròng'] = df_row_calc['Doanh thu cho thuê'] - df_row_calc['Chi phí vốn (theo khách)'] - df_row_calc['Tổng Chi Phí Sale'] - df_row_calc['Công ty'] - df_row_calc['Cá Nhân']

                # --- GỘP DỮ LIỆU SAU KHI TÍNH TOÁN ---
                # Định nghĩa rule gộp cho P&L
                pl_agg_rules = {
                    'Ngày ký': 'min', 'Ngày hết HĐ': 'max',
                    'Ngày in': 'min', 'Ngày out': 'max',
                    'Tổng giá trị HĐ': 'sum',
                    'Chi phí vốn (theo khách)': 'sum',
                    'Doanh thu cho thuê': 'sum',
                    'Tổng Chi Phí Sale': 'sum',
                    'Công ty': 'sum', 'Cá Nhân': 'sum',
                    'Lợi nhuận ròng': 'sum'
                }
                
                df_calc = df_row_calc.groupby(['Toà', 'Mã căn'], as_index=False).agg(pl_agg_rules)
        ...
        with tabs[7]:
            ...
            if not df_main.empty:
                # --- TÍNH TOÁN TRƯỚC ---
                df_cf_raw = df_main.copy()
                df_cf_raw['Thu: Thanh toán'] = df_cf_raw['KH thanh toán']
                df_cf_raw['Thu: Cọc'] = df_cf_raw['KH cọc']
                df_cf_raw['Chi: Chủ nhà'] = df_cf_raw['TT cho chủ nhà'] + df_cf_raw['Cọc cho chủ nhà']
                df_cf_raw['Chi: Hoa hồng'] = df_cf_raw['SALE THẢO'] + df_cf_raw['SALE NGA'] + df_cf_raw['SALE LINH'] + df_cf_raw['Công ty'] + df_cf_raw['Cá Nhân']
                
                # --- GỘP DÒNG ---
                cf_agg_rules = {
                    'Thu: Thanh toán': 'sum', 'Thu: Cọc': 'sum',
                    'Chi: Chủ nhà': 'sum', 'Chi: Hoa hồng': 'sum'
                }
                df_cf = df_cf_raw.groupby(['Toà', 'Mã căn'], as_index=False).agg(cf_agg_rules)
                ...
                # Tổng
                df_final_cf['TỔNG THU'] = df_final_cf['Thu: Thanh toán'] + df_final_cf['Thu: Cọc']
