...
if uploaded_key is not None:
    ...
    if sh:
        ...
        # ----------------------------------------------------------------------
        # SIDEBAR: TRUNG TÂM THÔNG BÁO (ĐÃ CẬP NHẬT THEO YÊU CẦU)
        # ----------------------------------------------------------------------
        with st.sidebar:
            st.divider()
                ...
                if df_hd.empty and df_kh.empty: 
                    ...
                    if not df_hd.empty:
                        ...
                        for _, r in df_hd.iterrows():
                             ...
                             # Lấy thông tin Tòa và Phòng
                             toa_nha = str(r['Toà']).strip() if str(r['Toà']).strip() != '' else "Chưa rõ tòa"
                             phong = str(r['Mã căn']).strip()
                             
                             # Hiển thị in đậm Tòa và Phòng
                             st.markdown(f"**🏠 {toa_nha} - P.{phong}**")
                             st.caption(f"⚠️ {status_msg} (Hết hạn: {r['Ngày hết HĐ'].strftime('%d/%m')})")
                             st.markdown("---") # Đường kẻ phân cách
                             
                    # --- HIỂN THỊ CẢNH BÁO KHÁCH SẮP OUT (CẬP NHẬT) ---
                    if not df_kh.empty:
                        ...
                        for _, r in df_kh.iterrows(): 
                            ...
                            # Hiển thị rõ ràng: Tòa + Phòng
                            st.markdown(f"**🚪 {toa_nha} - P.{phong}**")
                            st.caption(f"👤 {ten_khach} | ⏳ Còn {days_left} ngày")
                            st.caption(f"📅 Ngày out: {r['Ngày out'].strftime('%d/%m/%Y')}")
                            st.markdown("---") # Đường kẻ phân cách
            
            st.divider()
            if st.button("🔄 F5 - Tải lại dữ liệu", use_container_width=True): 
                st.cache_data.clear()
                st.rerun()
        ...
        # ----------------------------------------------------------------------
        # TAB 6: QUẢN LÝ CHI PHÍ (DETAIL VIEW) - KHÔNG GỘP DÒNG
        # ----------------------------------------------------------------------
        with tabs[5]:
            ...
            if not df_main.empty:
                ...
                # Ghi chú đơn giản cho từng dòng
                def make_note(row):
                ...
                st.dataframe(
                    ...
                    use_container_width=True, 
                    column_config={"Ghi chú": st.column_config.TextColumn(width="medium")}
                )
            else: st.info("Chưa có dữ liệu.")

        # ----------------------------------------------------------------------
        # TAB 7: TỔNG HỢP CHI PHÍ (P&L) - KHÔNG GỘP DÒNG
        # ----------------------------------------------------------------------
        with tabs[6]:
            ...
            if not df_main.empty:
                ...
                c_m1.metric("💰 TỔNG DOANH THU", fmt_vnd(df_calc['Doanh thu cho thuê'].sum()))
                c_m2.metric("📉 TỔNG CHI PHÍ & VỐN", fmt_vnd(df_calc['Chi phí vốn (theo khách)'].sum() + df_calc['Tổng Chi Phí Sale'].sum() + df_calc['Công ty'].sum() + df_calc['Cá Nhân'].sum()))
                c_m3.metric("💎 TỔNG LỢI NHUẬN", fmt_vnd(df_calc['Lợi nhuận ròng'].sum()))
                st.divider()
        ...
        # ----------------------------------------------------------------------
        # TAB 8: QUẢN LÝ DÒNG TIỀN - KHÔNG GỘP DÒNG
        # ----------------------------------------------------------------------
        with tabs[7]:
            ...
            if not df_main.empty:
                ...
                c_cf1.metric("💰 TỔNG THỰC THU", fmt_vnd(df_final_cf['TỔNG THU'].sum()))
                c_cf2.metric("💸 TỔNG THỰC CHI", fmt_vnd(df_final_cf['TỔNG CHI'].sum()))
                c_cf3.metric("💎 DÒNG TIỀN RÒNG", fmt_vnd(df_final_cf['DÒNG TIỀN RÒNG'].sum()))
                st.divider()
